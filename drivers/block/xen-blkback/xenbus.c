...
static void xen_update_blkif_status(struct xen_blkif *blkif)
{
	...
	for (i = 0; i < blkif->allocated_rings; i++) {
		ring = &blkif->rings[i];
		if (blkif->vbd.feature_multiqueue)
			snprintf(ring_name, TASK_COMM_LEN, "%s-%d", name, i);
		else {
			BUG_ON(i != 0);
			snprintf(ring_name, TASK_COMM_LEN, "%s", name);
		}
		...
		if (IS_ERR(ring->xenblkd)) {
			...
			xenbus_dev_error(blkif->be->dev, err,
					"start %s xenblkd", ring_name);
			return;
		}
	}
}

static struct xen_blkif_ring *xen_blkif_ring_alloc(struct xen_blkif *blkif, int nr_rings)
{
	struct xen_blkif_ring *rings, *ring;
		...
		unsigned int evtchn, unsigned int ring_idx)
{
	...
	err = bind_interdomain_evtchn_to_irqhandler(blkif->domid, evtchn,
			xen_blkif_be_int, 0, dev_name, ring);
	if (err < 0) {
			   ...
			   const struct xenbus_device_id *id)
{
	...
	err = xenbus_printf(XBT_NIL, dev->nodename,
			    "xen_blkback_max_queues", "%u", xen_blkback_max_queues);
	if (err)
		pr_debug("Error writing xen_blkback_max_queues\n");
...
static int connect_ring(struct backend_info *be)
{
	...
	err = xenbus_scanf(XBT_NIL, dev->otherend,
			...
			"%u", &max_queues);
	if (err < 0) {
		blkif->vbd.feature_multiqueue = 0; /* Fall back to single queue */
	} else {
		blkif->vbd.feature_multiqueue = 1;
		if (max_queues > xen_blkback_max_queues) {
			/* buggy or malicious guest */
			xenbus_dev_fatal(dev, err,
					"guest requested %u queues, exceeding the maximum of %u.",
					max_queues, xen_blkback_max_queues);
			return -1;
		}
	}

	blkif->rings = xen_blkif_ring_alloc(blkif,
			blkif->vbd.feature_multiqueue ? max_queues : 1);
	if (!blkif->rings)
		return -ENOMEM;

	if (blkif->vbd.feature_multiqueue == 0) {
